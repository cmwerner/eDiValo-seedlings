...
Counterfactual plots (not including nutrients, since they don't interact with the traits)
```{r trait-SLA-plots2}
...
m.sla.fit <- predict(m.sla.specific, new.data, allow.new.levels = TRUE, se.fit = TRUE)
...
ggplot(new.data, 
                  aes(x = sla.2, y = sla.fit, 
                      ...
                      fill = light)) + 
  facet_grid(nutrient~grazing) +
  geom_line(size = 1.2, aes(color = light)) + 
