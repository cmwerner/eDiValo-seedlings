...
Options for model distrubtions are: Poisson, negative binomial, Conway-Maxwell-Poisson, zero-inflated structures for those, and hurdle models. Starting with just negative binomial. 

```{r basic-models}
# negative binomial
m.nb1 <- glmmTMB(count.max ~ grazing*nutrient + (1|species) +
                   (1|block:plotid), 
...
Preliminary analysis of total biomass trait. Going to have to think about what distribution is appropriate--probably negative binomial, or an explicitly zero inflated model. This isn't working yet. 
```{r trait-biomass-analysis}
m.biomass.poiss <- nat.seedlings.traits %>% 
  glmer(count.max ~ grazing + nutrient + light + bm.tot +
          grazing:nutrient + grazing:light + grazing:bm.tot + 
          nutrient:light + nutrient:bm.tot + light:bm.tot + (1|block:plotid),
          family = poisson(link = "log"), .)
