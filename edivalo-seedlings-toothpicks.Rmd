...
### Emergence Analysis
Separate model for each species. We may only be able to get useful models for Plantago and Dianthus, but it's possible that models for Galium and/or Medicago will also converge. We can also explore combining these into models that include species id as an effect (probably having it interact with ).
...
surv.full <-  data.frame(month = month.rep1, block = block.rep1, plot = plot.rep1, species = species.rep1, id..1.6. = toothpick.rep1) %>%
  left_join(mark.alive, by = c('plot', 'species','month', 'id..1.6.'))

#  add column for sampling time, doy and the beginning of the interval in which a seedling could have died to 
# data set
...
```
# Analyses
```{r survival models, echo = F, warning = F, message = F}

# We might need to add another level of hierachy in the random term?

# Dianthus
dian.mod<-coxme(Surv(doy,event)~grazing*nutrient*light +(1|block/plot), data = subset(surv.full1, species == "dianthus car")
summary(dian.mod)
Anova(dian.mod, type = "II", test.statistic = "Chisq")

# Plantago
plan.mod<-coxme(Surv(doy,event)~grazing*nutrient*light +(1|block/plot), data = subset(surv.full1, species == "plantago lan")
summary(plan.mod)
Anova(plan.mod, type = "II", test.statistic = "Chisq")
