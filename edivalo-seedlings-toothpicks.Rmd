...
month.rep1 <- rep(month.list, each= length(plot.list), times = 36)

surv.full <- data.frame(month = month.rep1, plot = plot.rep1, species = species.rep1, id..1.6. = toothpick.rep1) %>%
  left_join(mark.seedlings, by = c('plot', 'species','id..1.6.', 'month'))

#  add column for sampling time, doy and the beginning of the interval in which a seedling could have diesd to 
# data set
